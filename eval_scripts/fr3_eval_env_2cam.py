#!/usr/bin/env python3
"""
FR3 eval environment for dit-policy inference — TWO CAMERAS.

Same as fr3_eval_env.py but subscribes to two image topics:
  cam0 → wrist camera  (image_topic_1, default /camera/camera_wrist/color/image_raw)
  cam1 → external cam  (image_topic_2, default /camera/camera_d405/color/image_raw)
"""
...
class FR3EvalNode2Cam(Node):
    def __init__(
        ...
        super().__init__("fr3_eval_env_2cam")

        self.lock = threading.Lock()
        self.latest_q:    Optional[np.ndarray] = None
        self.latest_dq:   Optional[np.ndarray] = None
        self.latest_g:    Optional[float]       = None
        self.latest_img0: Optional[np.ndarray]  = None  # wrist
        self.latest_img1: Optional[np.ndarray]  = None  # external
        ...
        self.create_subscription(Image,      image_topic_1,  self._cb_image0,   reliable_qos)
        self.create_subscription(Image,      image_topic_2,  self._cb_image1,   reliable_qos)
        ...
        with self.lock:
            ...
            if self._arm_index_map is not None:
                self.latest_q = np.array(
                    [msg.position[i] for i in self._arm_index_map], dtype=np.float32
                )
                if len(msg.velocity) == len(msg.position):
                    self.latest_dq = np.array(
                        [msg.velocity[i] for i in self._arm_index_map], dtype=np.float32
                    )
    ...
    def _cb_image1(self, msg: Image):
        try:
            img = ros_image_to_bgr(msg)
            with self.lock:
                self.latest_img1 = img
        except Exception as e:
            self.get_logger().warn(f"cam1 decode failed: {e}", once=True)
        ...
        while time.time() - t0 < timeout_sec:
            with self.lock:
                ready = (
                    ...
                    self.latest_img0 is not None and
                    self.latest_img1 is not None
                )
            if ready:
                return True
            if time.time() - t0 > 3.0:
                missing = []
                with self.lock:
                    if self.latest_q    is None: missing.append("joint_states")
                    if self.latest_g    is None: missing.append("gripper")
                    if self.latest_img0 is None: missing.append("cam0 (wrist)")
                    if self.latest_img1 is None: missing.append("cam1 (external)")
    ...
    def get_cam1(self):
        with self.lock:
            if self.latest_img1 is None:
                raise RuntimeError("No cam1 image yet")
            return self.latest_img1.copy()
...
class FR3EvalEnv2Cam:
    def __init__(
        self,
        node: FR3EvalNode2Cam,
        hz=10.0,
        dq_limit=0.15,
        dg_limit=0.02,
        settle_time=1.0,
        ...
        self.close_trigger            = 0.07   # g_target below this → close
        self.open_trigger             = 0.075  # g_target above this → open
    ...
    def _make_obs(self):
        ...
        img1 = self.node.get_cam1()
        return SimpleObs(observation={
            "images": {"cam0": img0, "cam1": img1},
    ...
    def reset(self):
        if not self.node.wait_until_ready(timeout_sec=15.0):
            with self.node.lock:
                missing = []
                if self.node.latest_q    is None: missing.append("joint_states")
                if self.node.latest_g    is None: missing.append("gripper")
                if self.node.latest_img0 is None: missing.append("cam0 (wrist)")
                if self.node.latest_img1 is None: missing.append("cam1 (external)")
            raise RuntimeError(f"FR3EvalEnv2Cam not ready after 15s. Missing: {', '.join(missing)}")
