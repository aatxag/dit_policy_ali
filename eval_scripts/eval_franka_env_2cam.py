...
cam1 → external camera (image_topic_2, default /camera/camera_ext/color/image_raw)
"""
...
class FR3EvalNode2Cam(Node):
    def __init__(
        ...
        self.create_subscription(Image, image_topic_1, self._cb_image0, sensor_qos)
        self.create_subscription(Image, image_topic_2, self._cb_image1, sensor_qos)
...
class FR3EvalEnv2Cam:
        ...
        if not self.node.wait_until_ready(timeout_sec=15.0):
            with self.node.lock:
                ...
                if self.node.latest_img0 is None:
                    missing.append("cam0 (wrist)")
                if self.node.latest_img1 is None:
                    missing.append("cam1 (external)")
    ...
    def step(self, action: np.ndarray) -> SimpleObs:
        ...
        q_cmd = np.clip(q_target, FR3_Q_LOW, FR3_Q_HIGH)
