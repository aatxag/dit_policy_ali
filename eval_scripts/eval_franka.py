#!/usr/bin/env python3
"""
Eval script for dit-policy — ABSOLUTE ACTIONS, ONE CAMERA.
...
Usage:
  python eval_fr3_joint8_abs_1cam.py path/to/checkpoint.ckpt \
...
from eval_franka_env import make_fr3_env_1cam
...
class Policy:
    def __init__(self, agent_path, model_name, args):
        ...
        self.img_keys = obs_config["imgs"]   # should be ["cam0"]
        self.pred_horizon = args.pred_horizon

        self.img_chunk = int(agent_config.get("imgs_per_cam", 1))
        if len(self.img_keys) != 1:
            print(f"[WARN] obs_config imgs={self.img_keys}; expected ['cam0'] for 1cam eval.")
...
def main():
    ...
    parser.add_argument("--image_topic", default="/camera/camera_wrist/color/image_raw", type=str)
